...
#if (QF_MAX_ACTIVE <= 8)
    extern QPSet8  volatile QK_readySet_;/** QK ready-set of active objects */
#else
    extern QPSet64 volatile QK_readySet_;/** QK ready-set of active objects */
#endif
...
*
* \note QK_schedExt_() must be always called with interrupts locked.
* The scheduler might unlock the interrupts internally, but always
* returns with interrupts locked.
*/
